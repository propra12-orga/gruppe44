 ...
 * 
 * @author Gruppe44
 * Dropdown-Menue mit einzelnen Auswahlmšglichkeiten wird erstellt
 *
 */
public class BombermanMenuBar extends JMenuBar {
	public BombermanMenuBar(){
		...
		data.add(names);
		data.add(getMapEditor());
		data.add(getMultiPlayer());
		data.add(getExit());
	 ...
	 * @return action 
	 */
	private AbstractAction getMultiPlayer() {
	 	AbstractAction action = new AbstractAction("Multiplayer") {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				MainGUI.panel.showMultiPlayer();
			}
		};
		return action;
	}
	 ...
	 * @return action 
	 */
	private AbstractAction getMapEditor() {
	 	AbstractAction action = new AbstractAction("Map-Editor") {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				MainGUI.panel.showMapEditor();
			}
		};
		return action;
	}
